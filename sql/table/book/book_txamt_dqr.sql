...
comment on column book_txamt_dqr.ys_type   is '原始凭证类型';
comment on column book_txamt_dqr.ys_id     is '原始凭证ID';
comment on column book_txamt_dqr.jzpz_id   is '原始凭证类型';
comment on column book_txamt_dqr.ts_c      is '创建时间';
