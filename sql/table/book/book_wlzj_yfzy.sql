...
comment on column book_wlzj_yfzy.ys_type   is '原始凭证类型';
comment on column book_wlzj_yfzy.ys_id     is '原始凭证ID';
comment on column book_wlzj_yfzy.jzpz_id   is '原始凭证类型';
comment on column book_wlzj_yfzy.ts_c      is '创建时间';

-- id序列
drop sequence seq_wlzj_yfzy;
create sequence seq_wlzj_yfzy as bigint start with 1 increment by 1 minvalue 1 no maxvalue no cycle cache 200 order;
