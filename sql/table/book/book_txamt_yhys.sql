...
comment on column book_txamt_yhys.ys_type   is '原始凭证类型';
comment on column book_txamt_yhys.ys_id     is '原始凭证ID';
comment on column book_txamt_yhys.jzpz_id   is '原始凭证类型';
comment on column book_txamt_yhys.ts_c      is '创建时间';

-- id序列
drop sequence seq_txamt_yhys;
create sequence seq_txamt_yhys as bigint start with 1 increment by 1 minvalue 1 no maxvalue no cycle cache 200 order;
